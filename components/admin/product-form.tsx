...
interface Product { page_content?: any;  id: string; title: string; slug: string; description: string; images: string[]; tags: string[]; category_id: string; is_active: boolean; vendor: string; seo_title: string; seo_description: string; variants: VariantInput[]; }
export default function ProductForm({ categories, product }: { categories: Category[], product?: Product }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const payload = { 
      id: product?.id, title: title.trim(), slug: slug.trim() || slugify(title), description: description.trim(), images, tags: tags.split(',').map((tag) => tag.trim()).filter(Boolean), category_id: categoryId, is_active: isActive, vendor, seo_title: seoTitle, seo_description: seoDescription, page_content: pageContent, 
